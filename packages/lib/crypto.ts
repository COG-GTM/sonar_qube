...
// Authentication tag length for GCM is 16 bytes
...
export const symmetricDecrypt = function (text: string, key: string) {
  ...
  const iv_from_ciphertext = Buffer.from(ivHex, OUTPUT_ENCODING);
  const authTag = Buffer.from(authTagHex, OUTPUT_ENCODING);
  const decipher = crypto.createDecipheriv(ALGORITHM, _key, iv_from_ciphertext);
  decipher.setAuthTag(authTag);
  let deciphered = decipher.update(encryptedText, OUTPUT_ENCODING, INPUT_ENCODING);
